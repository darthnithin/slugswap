...
const GET_BASE_URL =
  process.env.GET_API_BASE_URL || "https://services.get.cbord.com/GETServices/services/json";
...
export async function callGetApi<TParams, TResponse>(
  ...
  try {
    ...
    if (!response.ok) {
      throw new GetApiError(`GET API ${service}.${method} failed with HTTP ${response.status}`);
    }
...
export async function createPin(validatedSessionId: string, deviceId: string, pin: string) {
