...
OUT_DIR="$DASHBOARD_DIR/public/instagram-stories/end-of-quarter-mar-2026"
PORT="${PORT:-3310}"
HOST="${HOST:-127.0.0.1}"
LOCK_PATH="$DASHBOARD_DIR/.next/dev/lock"
...
npx dotenv-cli -e .env -- npx tsx scripts/instagram-stories/write-instagram-story-data.ts

# Build and serve in production mode to avoid Next.js dev indicator badges in exports.
npm run build -w @slugswap/dashboard >"$BUILD_LOG" 2>&1

node -e "require('fs').rmSync(process.argv[1], { force: true })" "$LOCK_PATH"
npm run start -w @slugswap/dashboard -- --port "$PORT" --hostname "$HOST" >"$SERVER_LOG" 2>&1 &
SERVER_PID=$!
